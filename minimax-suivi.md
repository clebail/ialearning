...
**Prochaine étape C++ (priorité au prochain coup) : brancher les évènements sur la grille pour rendre le jeu JOUABLE.** Clic sur le widget `WPuissance4` → déduire la **colonne** depuis le `x` du clic (cf. `paintEvent` : `cell`, `offsetX`) → `play(col)` → `repaint()`. Faire jouer en humain-vs-humain d'abord (les deux joueurs au clic), l'IA viendra après avec `minmaxAB`. *Rappel piège JS* : un coup = **une colonne (un nombre)**, pas une structure (`move = x`, pas `move.x`) ; colonne 0 est *falsy* en JS mais OK en C++ (`int`).

**Ensuite C++** : `evaluate` (contenu de fenêtre : `nbX`/`nbO`, fenêtre morte si 2 camps, bonus centre), `minmaxAB` + `height[]`/`undo`, puis gamesolver.org.
